...
- 인덱스 내부 구조는 B* 트리 형식으로 되어있음.   // B: BINARY 이진트리
...
// SELECT 많이 쓰는 테이블이면서 DML 적게 쓰는 테이블에 좋음
// 회원가입에 많이 씀 (게시글에는 부적당)
...
-- 양이 너무 적어서 속도 차가 확인이 안 됨.
SELECT * FROM EMPLOYEE
WHERE EMP_ID = 215;  -- 인덱스 사용 O

SELECT * FROM EMPLOYEE
WHERE EMP_NAME = '대북혼'; -- 인덱스 사용 X  // 위 인덱스에 없는 컬럼을 사용
