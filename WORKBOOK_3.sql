...
-- 2. 휴학중인 학생들의 이름과 주민번호를
-- 나이가 적은 순서로 화면에 출력하시오
...
-- 3. 주소지가 강원도나 경기도인 학생들 중에서
-- 1900년대 학번을 가진 학생들의 이름과, 학번, 주소를
-- 이름의 오름차순으로 출력하시오.
-- 단, 출력헤더에는 "학생이름", "학번", "거주지 주소"가 출력되도록 한다.
SELECT STUDENT_NAME 학생이름, STUDENT_NO 학번, STUDENT_ADDRESS "거주지 주소"
FROM TB_STUDENT
WHERE SUBSTR(STUDENT_NO, 1, 2) LIKE '9%'
...
-- (법학과의 학과코드는 학과 테이블(TB_DEPARTMENT)를 조회)
...
JOIN TB_CLASS_PROFESSOR USING(CLASS_NO)
JOIN TB_PROFESSOR USING(PROFESSOR_NO)
JOIN TB_DEPARTMENT DEPT ON(CLS.DEPARTMENT_NO = DEPT.DEPARTMENT_NO) -- ambiguously defined
WHERE CATEGORY = '인문사회';
...
-- 음악학과 학생들의 "학번", "학생 이름", "전체 평점"을 출력하는
-- SQL 문장을 작성하시오.
...
-- 지도 교수에게 내용을 전달하기 위한 학과 이름, 학생 이름과 지도 교수 이름이 필요하다. 이때 사용할 SQL문을 작성하시오.
...
-- (기존 워크북 PDF에 나타난 조회 결과는 DB 버전이 낮아 현재와 조회 방식이 다름.
-- 결과 행의 수만 동일하게 조회하자)
SELECT CLASS_NAME, DEPARTMENT_NAME  /*, PROFESSOR_NO, CATEGORY*/
FROM TB_CLASS
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
...
JOIN TB_GRADE USING(STUDENT_NO)
WHERE ABSENCE_YN = 'N'
GROUP BY STUDENT_NO, STUDENT_NAME, DEPARTMENT_NAME    -- 이 부분 주의!
HAVING AVG(POINT) >= 4.0
ORDER BY STUDENT_NO;


--SELECT STUDENT_NO 학번, STUDENT_NAME 이름, DEPART AVG(POINT) 평점
--FROM TB_GRADE
--JOIN TB_STUDENT USING(STUDENT_NO)
--GROUP BY STUDENT_NO, STUDENT_NAME
--HAVING AVG(POINT) >= 4.0;
...
-- 표시되도록 한다.
SELECT DEPARTMENT_NAME "계열 학과명", AVG(POINT) 전공평점
FROM TB_CLASS
LEFT JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
