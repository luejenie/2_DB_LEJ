...
-- 풀이
-- // 단일행 함수와 그룹함수가 같이 사용되어 조회된 행의 개수가 일치하지 않기 때문에 에러.
SELECT STUDENT_NO "학번", STUDENT_NAME "학생 이름", ROUND(AVG(POINT),1) "전체 평점"
												      	/*그룹함수*/
FROM TB_GRADE
JOIN TB_STUDENT USING(STUDENT_NO)
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
WHERE DEPARTMENT_NAME = '음악학과'
GROUP BY STUDENT_NO, STUDENT_NAME   --그룹화되지 않은 컬럼을 다 GROUP BY에 넣어주면 됨
ORDER BY 학번;
...
SELECT CLASS_NAME, DEPARTMENT_NAME
FROM TB_CLASS
LEFT JOIN TB_CLASS_PROFESSOR USING(CLASS_NO) -- LEFT JOIN 전 776개, 후 1000+
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)  
WHERE CATEGORY = '예체능'
AND PROFESSOR_NO IS NULL;


SELECT COUNT(*) FROM TB_CLASS;  --882개
...
-- 풀이2의 속도가 더 빠름. JOIN을 안해서 속도차이가 남.
-- 서브쿼리도 느리지만 JOIN이 더 느림.


/*
-- TB_GRADE에서 학번, 평균 조회 -> 국어국문학과 연결 -> STUDENT_NAME 추가  -- 여기까지하고 인라인뷰를 못썼네..
SELECT STUDENT_NO, STUDENT_NAME
FROM TB_GRADE
JOIN TB_STUDENT USING(STUDENT_NO)
WHERE DEPARTMENT_NO = ( 
...
-- : 그룹함수는 한 컬럼을 기준으로 2개까지 중첩하여 사용가능, 3개 이상 중첩하였을 경우 나타나는 에러
-->  서브쿼리를 이용하여 풀어서 쓰면 해결
...
-- ????
-- 19. 춘 기술대학교의 "환경조경학과"가 속한 같은 계열 학과들의 학과별 전공과목 평점을 파악하기 위한
-- 적절한 SQL문을 찾아내시오.
...
WHERE CATEGORY = (SELECT CATEGORY   -- 계열==자연과학
				  FROM TB_DEPARTMENT
				  WHERE DEPARTMENT_NAME = '환경조경학과')	
AND CLASS_TYPE LIKE '전공%'
