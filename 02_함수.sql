...
-- 단일 행 함수 : N개의 값을 읽어 N개의 결과를 반환  _DATA
-- 그룹 함수 : N개의 값을 읽어 1개의 결과를 반환(합계, 평균, 최대, 최소)  _INFROMATION 정보를 얻을 때
...
-- SUBSTR('문자열' | 컬럼명, 잘라내기 시작할 위치 [, 잘라낼 길이]  )     _INSRT이랑 주로 같이 씀
-- 컬럼이나 문자열에서 지정한 위치부터 지정된 길이만큼 문자열을 잘라내서 반환
--> 잘라낼 길이 생략 시 끝까지 잘라냄

-- EMPLOYEE 테이블에서 사원명, 이메일 중 아이디만 조회
-- sun_di@or.kr
SELECT EMP_NAME, SYBSTR(EMAIL, 1, INSTR(EMAIL, '@')-1) 아이디       -- 1에서 7까지 잘라냄. @포함되므로 -1. /별칭 적용
FROM EMPLOYEE;
...
SELECT TRIM('     H E L L O     ') 트림적용  --H E L L O 
FROM DUAL; -- 양쪽 공백 제거

SELECT TRIM('#' FROM '#####안녕#####')  --안녕
FROM DUAL;

SELECT TRIM(TRAILING '#' FROM '#####안녕#####')  -- LEADNIG, TRAILING, BOTH 적용
FROM DUAL;
...
SELECT EMP_NAME, SALARY, MOD(SALARY, 1000000)   --백만 단위가 사라짐
FROM EMPLOYEE;
...
SELECT ROUND(123.456, -1) FROM DUAL;  -- 소수점 0번째 자리에서 반올림해서, 소수점 -1 자리 표현
									  -- 120,  == 1의 자리를 반올림해서 10의 자리를 표현  (정수 부분도 반올림 가능)

SELECT ROUND(123.456, -2) FROM DUAL;  -- 100, 10의 자리를 반올림해서 100의 자리를 표현  



-- CEIL(숫자 | 컬럼명) : 올림
-- FLOOR(숫자 | 컬럼명) : 내림
--> 둘다 소수점 첫째 자리에서 올림/내림 처리     __소수점 자리 지정이 X
...
SELECT TRUNC(123.456) FROM DUAL;  -- 123, 소수정 아래 버림
SELECT TRUNC(123.456, 1) FROM DUAL;  -- 123.4, 소수정 첫째자리 아래 버림 (둘째자리부터 버림)
SELECT TRUNC(123.456, -1) FROM DUAL;  -- 120, 소수정 10의 자리(-1자리) 아래 버림 (1의자리부터 버림)
...
SELECT SYSDATE +120 FORM DUAL;    -- ADD_MONTH를 모르면 일수를 더해야 함. 정확하지 못함.
...
SELECT TO_CHAR(1234, '00000') FROM DUAL;  -- '01234'    __빈칸에 0을 채워넣음

SELECT TO_CHAR( EXTRACT (MONTH FROM HIRE_DATE), '00') || '월'     -- __ 자릿수 동일하게 맞출 수 있음
FROM EMPLOYEE;
...
SELECT SYSDATE FROM DUAL;  -- 2022-09-01 11:20:57.000

--2022/09/01 11:20:57 목요일 (출력화면)
...
-- 년, 월, 일이 날짜를 나타내는 패턴으로 인식이 안돼서 오류가 발생
...
SELECT '2022-09-02' FROM DUAL;   -- 탭에 ABC 써있음(문자형)
...
SELECT '1,000,000' + 500000 FROM DUAL;   -- ORA-01722: 수치가 부적합합니다
...
SELECT SUM(DECODE( SUBSTR(EMP_NO, 8, 1), '1', 1, 0 ))  -- 0은 아무리 더해도 0, COUNT로 숫자 못 셀 때
FROM EMPLOYEE;
