/*
    * SUBQUERY (서브쿼리)
    - 하나의 SQL문 안에 포함된 또다른 SQL(SELECT)문    _ SELECT만 가능. INSERT안에 INSERT..등 불가
...
/*  서브쿼리 유형
    ...
    - 단일행(단일열) 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 1개일 때   //조회결과가 1행 1열
    ...
    - 상관 서브쿼리 : 서브쿼리가 만든 결과 값을 메인 쿼리가 비교 연산할 때 
                     ...
                     // 일반 쿼리와 달리 메인쿼리가 먼저 해석된 후에 서브쿼리가 해석됨.
...
-- RETIREMENT 퇴직의 약어 == ENT


-- 2) 퇴사한 여직원과 같은 부서, 같은 직급 (다중열 서브쿼리)
...
-- (컬럼 순서가 중요!)
SELECT EMP_NAME, JOB_CODE, DEPT_CODE, HIRE_DATE, EMP_NO
FROM EMPLOYEE
...
-- // IN/NOT IN , >< ANY, >< ALL만 쓸 수 있음. 기본연산자X
...
-- 2) 직급별 평균 급여  // --천의자리까지 잘라냄. 만단위부터 나타남
...
-- 1) MAIN 1행의 DEPT_CODE를 서브쿼리에 대입
-- 2) SUB를 수행
...
-- 7. 인라인 뷰(INLINE-VIEW)
--    FROM 절에서 서브쿼리를 사용하는 경우로   // FROM 서브쿼리
...
WHERE 부서 = '기술지원부';    
-- DEPT_TITLE = '기술지원부'는 사용할 수 없음.
-- 해석순서에  따라 FROM절에서 사용한 별칭을 사용해야함.
...
-- 2) RANK() OVER(정렬순서)
SELECT RANK() OVER(ORDER BY SALARY DESC) 순위,   --'급여 내림차순 순으로 순위를 매겨라'
	   EMP_NAME, SALARY
FROM EMPLOYEE;
--19	전형돈	2000000
--19	윤은해	2000000
...
SELECT DENSE_RANK() OVER(ORDER BY SALARY DESC) 순위,   --'급여 내림차순 순으로 순위를 매겨라'
	   EMP_NAME, SALARY
FROM EMPLOYEE;
--19	전형돈	2000000
--19	윤은해	2000000
