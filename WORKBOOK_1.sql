...
SELECT STUDENT_NAME
FROM TB_STUDENT
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)   --학과코드
...
-- 4. 도서관에서 대출 도서 장기 연체들을 찾아 이름을 게시하려고 한다.
-- 	  그 대상자들의 학번이 다음과 같을 때 대상자들을 찾는 적절한 SQL 구문 작성하기
SELECT STUDENT_NAME 
FROM TB_STUDENT
...
-- 7. 전산상의 착오로 학과가 지정되지 않은 학생 있는지 확인하고자 한다.
--	  어떤 SQL 문장을 사용하면 되는지 작성하시오.
SELECT STUDENT_NAME
FROM TB_STUDENT 
WHERE DEPARTMENT_NO IS NULL;
