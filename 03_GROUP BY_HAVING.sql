...
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', 2, '여') 성별, 
	...
	SUM(SALARY) "급여 합"	
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', 2, '여')   -- 별칭 사용X(해석 순서에 따라서 별칭 해석X)
ORDER BY "인원 수";  -- 별칭 사용O (SELECT절 해석 완료)
...
-- (직급코드 오름차순)
SELECT JOB_CODE 직급, SUM(SALARY) 합계
FROM EMPLOYEE
WHERE EXTRACT(YEAR FROM HIRE_DATE) >= 2000
-- WHERE HIRE_DATE >= TO_DATE('2000-01-01')     // TO_DATE는 쓰지 않아도, 자동형변환처럼 저절로 날짜로 인식해줌.
...
FROM EMPLOYEE
-- WHERE SALARY >= 3000000 -- 한 사람의 급여가 300만 이상이라는 조건(문제 요구사항 충족X)
GROUP BY DEPT_CODE 
...
-- CUBE : GROUP BY절에서 작성된 모든 컬럼의 중간 집계를 처리하는 함수
SELECT DEPT_CODE, JOB_CODE, COUNT(*) 
FROM EMPLOYEE
...
--(!주의사항!) 집합 연샂나를 사용하기 위한 SELECT문들은
-- 조회하는 컬럼의 타입, 개수가 모두 동일해야 한다!
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY 
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION
SELECT EMP_ID, EMP_NAME, DEPT_CODE, '1'  --위 SALARY는 숫자, '1'은 문자라 에러 
FROM EMPLOYEE
WHERE SALARY > 3000000;
...
-- __서로 관련이 없는 다른 테이블인데 합쳐서 볼 수 있음. ex) 포털사이트 통합검색 등에 사용됨
