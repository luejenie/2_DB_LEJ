...
/* 자료형
    ...
    CHAR(크기) : 고정길이 문자형 (2000BYTE)    
        ...
        // 전화번호 등 정해져 있는 문자열에 사용
        // CHAR2는 없음
        
    VARCHAR2(크기) : 가변길이 문자형 (4000 BYTE)  
        -> ex) VARCHAR2(10) 컬럼에 'ABC' 3BYTE 문자열만 저장하면 나머지 7BYTE를 반환함.
        // 이름, 주소, 자기소개 등 정해져 있지 않은 문자열에 사용
        // VARCHAR *2 크기를 늘린 것. VARCHAR는 사용하지 않음.
        
    DATE : 날짜 타입
    BLOB : 대용량 이진 데이터 (4GB) // 글이 아닌 형태(동영상 등)도 저장할 수 있음 
    								-> 지양함. 같은 데이터베이스를 사용하는 사람들의 속도가 느려지기 때문에.
    CLOB : 대용량 문자 데이터 (4GB)
...
CREATE TABLE "MEMBER"(   
	...
	MEMBER_SSN CHAR(14),  -- 14글자 고정
	ENROLL_DATE DATE DEFAULT SYSDATE   --ENROLL_DATE는 기본적으로 현재시간이다.
);

-- // MEMBER라는 컬럼이 있어서 색이 변하지만 상관은 없음. 헷갈리면 "" 붙이기
...
-- '2022-09-13 17:33:27'
INSERT INTO "MEMBER" VALUES('MEM05', 'PASS05', '김길동', '930303-1234567',
...
/*
    ...
    제약조건 종류  // 5가지 모두 외우고, 작성법 모두 알아둬야 함!
...
-- //무조건 값을 넣어야 함. ID, 비밀번호, 게시글에서 제목, 작성자 등등
...
-- ORA-01400: NULL을 ("KH_LEJ"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_UK(
    USER_NO NUMBER,
--    USER_ID VARCHAR2(20) UNIQUE, -- 컬럼 레벨(제약조건명 미지정) // 자동부여 SYS~~  
...
-- ORA-00001: 무결성 제약 조건(KH_LEJ.USER_ID_U)에 위배됩니다
...
-- ORA-00001: 무결성 제약 조건(KH_LEJ.USER_ID_NAME_U)에 위배됩니다
...
-- 한 개 컬럼에 설정할 수도 있고, 여러개의 컬럼을 묶어서 설정할 수 있음 // (복합키 설정도 가능)
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(KH_LEJ.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_LEJ"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다


---------------------------------------

-- PRIMARY KEY 복합키 (*테이블 레벨*만 가능)
...
-- ORA-00001: 무결성 제약 조건(KH_LEJ.PK_USERNO_USERID)에 위배됩니다
...
-- 제공되는 값 외에는 NULL을 사용할 수 있음 --> NULL : 참조하는 값이 없음
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT GARDE_CODE_FK REFERENCES USER_GRADE /*(GRADE_CODE)*/ -- 컬럼 레벨
 											-- 컬럼명 미작성 시 USER_GRADE 테이블의 PK를 자동 참조
  -- 테이블 레벨
  --, CONSTRAINT GARDE_CODE_FK FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE 
...
-- ORA-02291: 무결성 제약조건(KH_LEJ.GARDE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다 (부모 테이블 : USER_GRADE)
