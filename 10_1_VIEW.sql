/* VIEW
	...
	***** VIEW 사용 시 주의 사항 *****
	...
	2) VIEW를 이용한 DML(INSERT/UPDATE/DELETE)가 
	   가능한 경우도 있지만
	   많은 제약이 따르기 떄문에 SELECT용도로 사용하는 것을 권장
  ...
  --** 1) OR REPLACE 옵션 : 기존에 동일한 뷰 이름이 존재하는 경우 덮어쓰고, 
    --                      존재하지 않으면 새로 생성.     //없으면 만들고(CREATE) 있으면 교체하고(OR REPLACE)
    -- 2) FORCE / NOFORCE 옵션
    --      FORCE : 서브쿼리에 사용된 테이블이 존재하지 않아도 뷰 생성
    --      NOFORCE : 서브쿼리에 사용된 테이블이 존재해야만 뷰 생성(기본값)
    -- 3) WITH CHECK OPTION 옵션 : 옵션을 설정한 컬럼의 값을 수정 불가능하게 함.  // READ ONLY의 하위호환. 잘안씀
  --** 4) WITH READ ONLY 옵션 : 뷰에 대해 조회만 가능(DML 수행 불가)   		// 더 많이 사용

*/
--// CREATE OR REPLACE 통째로 외워라
...
--> 가상의 테이블인 VIEW에 데이터 삽입이 가능한걸까?  NO   // VIEW가 아니라 원본테이블에 저장됨.
...
-- // 현재 원본테이블 DEPT_TITLE이 NOT NULL 상태이기 때문에 삽입 불가
...
--> ORA-01400: NULL을 ("KH_LEJ"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다

-- VIEW의 본래 목적인 보여지는 것(조회)이라는 용도에 맞게 사용하는 게 좋다.
--> WITH READ ONLY 옵션 사용  // 읽기전용으로 만들어서 아예 수정을 못하게끔.
