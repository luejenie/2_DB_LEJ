...
-- 3) 수정은 별도 존재하지 않음! -> 삭제 후 추가를 이용해서 수정 
...
-- DEPT_COPY의 DEPT_TITLE 컬럼에 UNIQUE 삭제  //제약조건명을 알아야 삭제할 수 있음.
ALTER TABLE DEPT_COPY
...
-- ALTER TABLE 테이블명 DROP COLUMN 삭제할컬럼명;  // 위와 동일한 의미. 작성법만 차이
...
-- ORA-12899: "KH_LEJ"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
-- DEPT_ID -> CHAR[2]
...
--> 기본값을 변경했다고 해서 기존 데이터가 변하지는 않음   -- 그대로 '한국'
...
-- DEPT_COPY 테이블에 PK 추가(컬럼: DEPT_ID, 제약조건명: D_COPY_PK)  // DEPT_ID 컬럼에 D_COPY_PK이름의 기본키 추가
...
COMMIT;
--// TB2 테이블에는 1,2,3, +NULL만 들어갈 수 있음
...
--ORA-02291: 무결성 제약조건(KH_LEJ.SYS_C008437)이 위배되었습니다- 부모 키가 없습니다
...
/* DDL 주의 사항 */  --CREATE, ALTER, DROP
-- 1) DDL은 COMMINT/ROLLBACK이 되지 않는다.
--  -> ALTER, DROP을 신중하게 진행해야함.

-- 2) DDL과 DML 구문을 섞어서 수행하면 안된다!
--> DDL은 수행 시 존재하고 있는 트랜잭션을 모두 DB에 강제 COMMIT 시킴.  //밀고들어감.
...
ROLLBACK;  -- 컬럼명 변경한거 ROLLBACK이 안됨.
SELECT * FROM TB2;
