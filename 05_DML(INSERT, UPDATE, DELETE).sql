...
--------------------------------------------------------------------------------------------------------------------
-- SELECT, INSERT, UPDATE, DELETE 는 무조건 외워야 함!!

-- 윈도우 > 설정 > 연결 > 연결유형 > Auto-Commit by default 체크 해제  // 체크하면 자동으로 데이터에 반영
...
-- 1)  INSERT INTO 테이블명 VALUES(데이터, 데이터, ...)  : '테이블명' 안에 삽입하겠다.
-- 테이블에 모든 컬럼에 대한 값을 INSERT할 때 사용
-- INSERT하고자 하는 컬럼이 모든 컬럼인 경우 컬럼명 생략 가능. 
-- 단, 컬럼의 순서를 지켜서 VALUES에 값을 기입해야 함

INSERT INTO EMPLOYEE2 VALUES('900', '장채현', '901230-2345678', 'jang_ch@kh.or.kr', 
...
INSERT INTO EMPLOYEE2(EMP_ID, EMP_NAME, EMP_NO, EMAIL, PHONE, 
                      DEPT_CODE, JOB_CODE, SAL_LEVEL, SALARY)   --뒤의 값을 테이블명에 넣겠다
...
-- 위 서브쿼리(SELECT) 결과를 EMP_01 테이블에 INSERT
...
--> WHERE 조건 중요!
-- __ 바꿀 값으로 컬럼명을 세팅하고, 테이블을 수정하겠다. 일괄수정 -> 조건절을 추가해야함! WHERE절이 중요!
-- __ UPDATE EMPLOYEE SET EMP_NAME = '소지섭'

-- DEPARTMENT2 테이블에서 DEPT_ID가 'D9'인 부서 정보 조회
SELECT * FROM DEPARTMENT2;  -- 조회
...
-- _제일 많이 사용 + 제일 많이 오류내는 부분!
-- * 여러 컬럼을 한번에 수정할 시 콤마(,)로 컬럼을 구분하면됨.
-- D9 / 전략기획팀  -> D0 / 전략기획2팀으로 수정

UPDATE DEPARTMENT2 SET
DEPT_ID = 'DO',
DEPT_TITLE = '전략기획2팀'   --WHERE절 직전 컬럼 작성 후 콤마 XXXXXXX
WHERE DEPT_ID = 'D9'
AND DEPT_TITLE = '전략기획팀';
-- AND DEPT_TITLE = '총무부'  --DROP했다가 다시 만든 사람들만.
...
SELECT * FROM EMP_M02; -- 5명(기존 4명 + 신규 1명)__ 4명이 중복. 1번테이블은 급여O, 2번테이블은 급여X
