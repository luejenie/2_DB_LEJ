...
-- 부서명, 지역명 조회
-- ※ LOCATION 테이블 : LOCAL_CODE 지역코드 / NATIONAL_CODE 국가코드 / LOCAL_NAME 국가코드
-- ※ DEPARTMENT 테이블 : LOCAIONT_ID 지역코드 / DEPT_ID 부서코드 / DEPT_TITLE 부서명 
...
--> JOIN 구문을 잘못 작성하는 경우 CROSS JOIN의 결과가 조회됨   ___확인용
...
-- '='(등호)를 사용하지 않는 조인문
--  지정한 컬럼 값이 일치하는 경우가 아닌, 값의 범위에 포함되는 행들을 연결하는 방식   __ = 가 아닌 다른 게 들어감 / 국세청 등에서 많이 쓰임. (교차조인, 비등가조인은 많이 쓰이지는 않음)
...
SELECT EMP_NAME, SALARY, SAL_GRADE.SAL_LEVEL    --SAL_LEVEL이 SAL_GRADE랑 EMPLOYEE 두 군데 다 있음
FROM EMPLOYEE
...
FROM EMPLOYEE
--JOIN JOB USING(JOB_CODE);
NATURAL JOIN JOB;    -- 두 테이블이 타입과 컬럼명이 동일.
...
SELECT * 
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)      --_조인한 모든 컬럼을 보겠다. 맨 뒤에 붙음
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE);
...
--> 해결 방법 : DEPARTMENT와 LOACTION 조인 순서를 바꿔서
...
-- 사번, 이름, 직급명(JOB_NAME), 부서명(DEPT_TITLE), 근무지역명(LOCAL_NAME), 급여를 조회하세요

-- ANSI
-- 직급 대리 : JOB(JOB_NAME, JOB_CODE), EMPLOYEE(JOB_CODE)
-- 부서명 : DEPARTMENT(DEPT_TITLE, DEPT_ID), EMPLOYEE(DEPTT_CODE)
-- 근무지역명 : LOCATION(LOCAL_NAME, LOCAL_CODE), DEPARTMENT(LOCATION_ID)
-- 급여 : EMPLOYEE(SALARY)
...
--WHERE DEPT_TITLE = '해외영업1부' OR DEPT_TITLE = '해외영업2부';   --신뢰성이 다소 떨어짐
...
--!! 부서가 없는데 보너스는 받는 사람이 있음! 하동운  -> INNER JOIN이 아닌 LEFT JOIN으로 DEPARTMENT, LOCAION 바꿔야 함.
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)  -- WHERE절 없어도 됨. INNER JOIN 특징으로 이미 이 부분에서 부서 없는 사람 다 걸러짐.
JOIN LOCATION ON(LOCAL_CODE = LOCATION_ID);
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCAL_CODE = LOCATION_ID)
JOIN NATIONAL USING (NATIONAL_CODE)   --NATIONAL이라는 예약어가 있음. 하지만 JOIN과 같이 쓰면 테이블명으로 인식. 거슬리면 ""쓰기
WHERE NATIONAL_NAME IN ('한국', '일본');
-- WHERE NATIONAL_CODE IN ('KO', 'JP');

--WHERE NATIONAL_NAME LIKE '한국' 
--OR NATIONAL_NAME LIKE '일본';
...
WHERE E.EMP_NAME != P.EMP_NAME -- 자신이 자신의 동료라고 나옴. --> 제거해야 함
ORDER BY 1;  
...
--((JOB_CODE LIKE 'J4') OR (JOB_CODE LIKE 'J7'));
